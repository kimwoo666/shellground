...
class TerminalWidget(QWidget):
    ...
    def __init__(self, family='monospace'):
        ...
        self.setAccessibleName('실제 Linux 터미널')
    ...
    def paintEvent(self, event):
        ...
        for row in range(self.screen.lines):
            for col in range(self.screen.columns):
                ...
                wide = col + 1 < self.screen.columns and self.screen.buffer[row][col + 1].data == ''
        ...
        if self.hasFocus() and self.connected and not self.screen.cursor.hidden:
    ...
    def keyPressEvent(self, event):
        ...
        if shift and key in (Qt.Key.Key_PageUp, Qt.Key.Key_PageDown):
            (self.screen.prev_page if key == Qt.Key.Key_PageUp else self.screen.next_page)()
            self.update(); return
        if not self.connected: return
        while self.screen.history.position < self.screen.history.size: self.screen.next_page()
        mapping = {
    ...
    def inputMethodEvent(self, event):
        if self.connected and event.commitString(): self.input_bytes.emit(event.commitString().encode('utf-8'))
        event.accept()
    ...
    def copy_selection(self):
        ...
        for row in range(start[0], end[0] + 1):
            ...
            lines.append(''.join(self.screen.buffer[row][col].data for col in range(left, right + 1)).rstrip())
    ...
    def wheelEvent(self, event):
        (self.screen.prev_page if event.angleDelta().y() > 0 else self.screen.next_page)()
        self.update()
